...
public class RptRatingProyectadoConstructora implements SEFEReportePOI {
	...
	private void obtenerInformacionReporte(HSSFSheet planilla) {
		...
		obtenerIndicadoresFinancierosProyectados();
		imprimirEncabezadoReporte(planilla);
		imprimirIndicadoresFinancierosProyectados(planilla);
		imprimeTemasFinancieros(planilla);
	}

	/**
	 * Obtiene el Rating Financiero
	 */
	...
	private void obtenerIndicadoresFinancierosProyectados(){
	...
	private void imprimirEncabezadoReporte(HSSFSheet planilla) {
		...
		servicioRpt.insertarValorEnCelda(planilla, INDEX_FILA_ENCABEZADO, INDEX_COLUMNA_ENCABEZADO_3, FormatUtil.castBigDecimal(getRatingFinancieroProy().getNotaFinancieraSinAjuste()));
		...
		servicioRpt.insertarValorEnCelda(planilla, INDEX_FILA_ENCABEZADO + 3, INDEX_COLUMNA_ENCABEZADO_3, getRatingIndividual().getFechaCambioEstado() != null ? FormatUtil.formatDate(getRatingIndividual().getFechaCambioEstado()) : null);
		...
		// Segmento
		servicioRpt.insertarValorEnCelda(planilla, 11, 4,  getRatingIndividual().getFechaAvance() != null ? FormatUtil.formatDate(getRatingIndividual().getFechaAvance() ) : "-");
	}
	
	private void imprimirIndicadoresFinancierosProyectados(HSSFSheet planilla) {
	...
	private void imprimeTemasFinancieros(HSSFSheet planilla) {
		...
		for(int i = 0; i < temas.size() ; i++){
			...
			servicioRpt.insertarValorEnCelda(planilla, INDEX_FILA_TEMAS + i, INDEX_COLUMNA_TEMAS, FormatUtil.castBigDecimal(tema.getNumeroTema()));
